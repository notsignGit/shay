...
const criticalIndices = [10, 12, 29, 31, 32, 33];
const results = [];

function combs(items, size) {
  const out = [];
...
for (const combo of combs(criticalIndices, 2)) {
  const candidate = members.filter((_, index) => !combo.includes(index));
  const result = analyze(candidate);
  results.push({ combo, feasible: result.feasible, minSafetyFactor: result.minSafetyFactor });
}

results.sort((a, b) => a.minSafetyFactor - b.minSafetyFactor);
