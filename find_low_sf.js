...
function analyze(currentMembers) {
  ...
  const result = { feasible: true, minSafetyFactor, minNode, memberForces };
  analysisCache.set(key, result);
  return result;
}

const base = analyze(members);
const singleResults = [];
for (let index = 0; index < members.length; index += 1) {
  ...
  singleResults.push({
    ...
    minNode: result.minNode,
  });
}

singleResults.sort((a, b) => a.minSafetyFactor - b.minSafetyFactor);

console.log(JSON.stringify({
  currentMinSafetyFactor: base.minSafetyFactor,
  lowestSingle: singleResults.slice(0, 20),
  countBelow12: singleResults.filter((item) => item.minSafetyFactor < 1.2).length,
  countBelow1: singleResults.filter((item) => item.minSafetyFactor < 1).length,
}, null, 2));
